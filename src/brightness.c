...
float get_legacy_brightness() {
  ...
  if (result == kIOReturnSuccess) {
    ...
    while (service) {
      float temp = -1.0;
      kern_return_t status = IODisplayGetFloatParameter(
          service, kNilOptions, CFSTR(kIODisplayBrightnessKey), &temp);
...
int main(int argc, char *argv[]) {
  ...
  if (b < 0.0) {
    // Fallback for headless or virtualized machines where no physical display
    // exists
    if (level >= 0.0) {
      printf("%.2f\n", level);
    } else {
